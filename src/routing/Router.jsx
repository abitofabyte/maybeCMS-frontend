...
function getHeader(method, user) {
	return {
		method,
...
async function fetchDataForUser(user) {
	if (!user) return null
	const headers = getHeader("GET", user)
	console.log(headers)
	return fetch(config.routing.backend.products + "/admin", { headers })
}
